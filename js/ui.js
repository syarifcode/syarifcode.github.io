    ...
    async renderDramaDetail(dramaId) {
        ...
        try {
            ...
            if (cached) {
                ...
                // Try to get drama data from available endpoints
                // Note: getRandomDrama() returns a single drama object, not an array
                try {
                    const randomDramaData = await this.api.getRandomDrama();
                    
                    // Check if the randomDramaData is an array or object
                    if (Array.isArray(randomDramaData)) {
                        // If it's an array, find the drama by id
                        drama = randomDramaData.find(d => d.bookId === dramaId) || null;
                    } else if (randomDramaData && randomDramaData.bookId) {
                        // If it's a single drama object, check if it's the one we want
                        if (randomDramaData.bookId === dramaId) {
                            drama = randomDramaData;
                        } else {
                            drama = null;
                        }
                    }
                } catch (error) {
                    console.warn('Error fetching random drama:', error);
                    drama = null;
                }
                
                if (!drama) {
                    // If not found, try to get from trending or foryou
                    try {
                        const trendingData = await this.api.getTrendingDramas();
                        if (Array.isArray(trendingData)) {
                            drama = trendingData.find(d => d.bookId === dramaId) || null;
                        }
                        
                        if (!drama) {
                            const forYouData = await this.api.getForYouDramas();
                            if (Array.isArray(forYouData)) {
                                drama = forYouData.find(d => d.bookId === dramaId) || null;
                            }
                        }
                    } catch (error) {
                        console.warn('Error fetching from other endpoints:', error);
                    }
                }
                
                if (!drama) {
                    // If still not found, create a mock drama object with minimal data
                    drama = {
                        bookId: dramaId,
                        bookName: 'Drama',
